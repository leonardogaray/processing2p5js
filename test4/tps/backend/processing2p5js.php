...
function processing2p5js($containerName, $pde){
    ...
    $pFunctions = "/((resizeCanvas|sin|cos|endShape|vertex|bezierDetail|pop|push|rotate|textAlign|bezier|translate|random|beginShape|textSize|quad|strokeCap|point|colorMode|smooth|rectMode|line|circle|ellipseMode|fill|ellipse|createCanvas|background|noFill|stroke|noStroke|triangle|strokeWeight|text|rect|arc|radians|loadImage|image|dist)(\s){0,}\()/";
    $pVars = "/\s(frameCount|BOTTOM|CLOSE|SQUARE|CORNERS|CORNER|ROUND|mouseX|mouseY|CENTER|TWO_PI|HALF_PI|PIE|PI|QUARTER|RGB|width|height|HSB)/";
    ...
    $p5jsReturn = preg_replace("/new PVector \((\w+,.\w+)\)/","createVector($1)", $p5jsReturn);
    $p5jsReturn = preg_replace("/\b(int|float|PImage|boolean)\s\[\]/","var ", $p5jsReturn);
    $p5jsReturn = preg_replace("/\b(color|int|float|boolean|String|Char|PImage|long|PVector)\s/","var ", $p5jsReturn);
    ...
    $p5jsReturn = preg_replace($pFunctions,"\n p.$1", $p5jsReturn);
