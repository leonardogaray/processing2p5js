...
function processing2p5js($containerName, $pde){
    ...
    $pFunctions = "/((tint|resizeCanvas|sin|cos|endShape|vertex|bezierDetail|pop|push|rotate|textAlign|bezier|translate|random|beginShape|textSize|quad|strokeCap|point|colorMode|smooth|rectMode|line|circle|ellipseMode|fill|ellipse|createCanvas|background|noFill|stroke|noStroke|triangle|strokeWeight|text|rect|arc|radians|loadImage|image|dist|loadFont|loadText|textFont|noCursor|cursor|loop|noLoop|map|noise|imageMode)(\s){0,}\()/";
    $preVars = "(\=|\s|\+|\-|\*|\/|\(|\;|\,)";
    $postVars = "(\=|\s|\+|\-|\*|\/|\)|\;|,)";
    $pVars = "/".$preVars."?(frameCount|BOTTOM|CLOSE|SQUARE|CORNERS|CORNER|ROUND|mouseX|mouseY|CENTER|TWO_PI|HALF_PI|PIE|PI|QUARTER|RGB|width|height|HSB|LEFT_ARROW|RIGHT_ARROW|UP_ARROW|DOWN_ARROW|key|RIGHT|LEFT|TOP)".$postVars."/";
    $pMainFunctions = "setup|draw|keyPressed|mousePressed|mouseClicked";
    ...
    $p5jsReturn = preg_replace("/".$preVars."LEFT".$postVars."/","$1LEFT_ARROW$2", $p5jsReturn);
    $p5jsReturn = preg_replace("/".$preVars."RIGHT".$postVars."/","$1RIGHT_ARROW$2", $p5jsReturn);
    $p5jsReturn = preg_replace("/".$preVars."UP".$postVars."/","$1UP_ARROW$2", $p5jsReturn);
    $p5jsReturn = preg_replace("/".$preVars."DOWN".$postVars."/","$1DOWN_ARROW$2", $p5jsReturn);
