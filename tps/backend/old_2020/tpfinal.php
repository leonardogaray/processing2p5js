...
try {
    ...
    $where = " ass.id IN (44) "; //Id from TP - moodle.mdl_assign
    ...
    foreach($result as $index => $row){
        ...
        $repo = preg_replace("/https\:\/\/github.com\/(\w+)\/(\w+)\/tree\/tpfinal/", '$1', $github);
        $project = preg_replace("/https\:\/\/github.com\/(\w+)\/(\w+)\/tree\/tpfinal/", '$2', $github);
